...
export default function MapCanvas({
  ...
  return (
    <div className="flex flex-col h-full min-h-[550px] bg-slate-900 border border-slate-800 rounded-none overflow-hidden relative shadow-2xl">
      ...
      <div className="flex flex-wrap justify-between items-center px-6 py-3.5 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md z-10 gap-3">
        <div className="flex items-center gap-3">
          <span className="w-8 h-8 p-1 bg-white rounded-none border border-slate-200 flex items-center justify-center overflow-hidden">
            {currentProject?.logo ? (
          ...
          <div>
            <div className="flex items-center gap-2 font-bold text-sm text-white font-outfit">
              ...
              <span className="text-[10px] bg-blue-950 text-blue-400 border border-blue-800/40 px-2 py-0.5 rounded-none uppercase tracking-wider">
      ...
      >
          ...
          <div className="absolute top-4 left-6 z-20 px-3.5 py-2 bg-slate-900/90 border border-slate-800/90 rounded-none backdrop-blur-md text-xs text-slate-200 flex items-center gap-2.5 shadow-lg">
            {currentProject.logo && (
              <img
                ...
                className="w-7 h-7 rounded-none object-contain bg-white p-0.5 border border-slate-700 shrink-0"
              />
            )}
            <div>
        ...
        {mapRenderMode === 'image' && !imageLoaded && (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center bg-slate-950/90 z-20 backdrop-blur-sm">
            <div 
              ...
              className="border-2 border-dashed border-slate-800 hover:border-blue-500 hover:bg-blue-950/10 p-10 rounded-none cursor-pointer max-w-lg transition-all"
            >
        ...
        {tooltip.show && (
          <div 
            className="absolute z-30 pointer-events-none p-3.5 bg-slate-900/95 border border-slate-700/80 rounded-none shadow-2xl text-xs flex flex-col gap-1.5 min-w-[170px] text-white backdrop-blur-md"
        ...
        {/* Zoom controls panel */}
        <div className="absolute bottom-6 right-6 flex flex-col gap-2 p-1.5 bg-slate-900/90 border border-slate-800/90 rounded-none backdrop-blur-md z-20 shadow-lg">
          <button 
            onClick={zoomIn}
            className="w-9 h-9 flex items-center justify-center text-slate-300 hover:text-white hover:bg-blue-600 rounded-none transition-colors cursor-pointer"
            title="Zoom In"
          >
