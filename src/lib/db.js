  ...
  if (supabase) {
    try {
      ...
      return data.map(item => ({
        ...
        plotType: item.plot_dimensions || item.plot_type || '',
        status: item.payment_status || item.status || 'token_received',
        ...
        developmentCharges: parseFloat(item.development_charges) || 0,
        date: item.date,
  ...
  if (supabase) {
    try {
      const fullPayload = {
        ...
        father_name: booking.fatherName || booking.relativeName || null,
        relative_name: booking.fatherName || booking.relativeName || null,
        cnic: booking.cnic || null,
        phone: booking.phone,
        email: booking.email || null,
        block: booking.block || null,
        plot_dimensions: booking.plotDimensions || booking.plotType || null,
        payment_mode: booking.paymentMode || 'Cash',
        bank_name: booking.bankName || null,
        plot_type: booking.plotDimensions || booking.plotType || null,
        status: booking.paymentStatus || booking.status,
        payment_status: booking.paymentStatus || booking.status,
        total_price: booking.totalPayable || booking.totalPrice || 0,
        total_payable: booking.totalPayable || booking.totalPrice || 0,
        paid_amount: booking.amountReceived || booking.paidAmount || 0,
        amount_received: booking.amountReceived || booking.paidAmount || 0,
        cost_of_land: booking.costOfPlot || booking.costOfLand || null,
        extra_charges: booking.extraCharges || null,
        processing_charges: booking.processingCharges || null,
        development_charges: booking.developmentCharges || null,
        date: booking.date,
